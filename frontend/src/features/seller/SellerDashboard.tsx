...
export default function SellerDashboard() {
  ...
  const [tgBannerDismissed, setTgBannerDismissed] = useState(() => {
    return localStorage.getItem('tg_banner_dismissed') === '1';
  });

  const handleDismissTgBanner = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {/* Telegram Banner */}
      {!user.has_telegram && !tgBannerDismissed && (
        <div className="mb-6 bg-gradient-to-r from-sky-50 to-blue-50 border border-sky-200 rounded-2xl p-4 flex items-center gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-sky-900">
              Рекомендуем привязать Telegram
            </p>
            <p className="text-xs text-sky-700 mt-0.5">
              Получайте уведомления о заказах и сможете восстановить пароль
            </p>
          </div>
          <a
            href={telegramBotUrl}
            ...
            className="flex-shrink-0 px-4 py-2 text-sm font-medium text-white bg-sky-500 hover:bg-sky-600 rounded-xl transition-colors"
          >
            Привязать
          </a>
          <button
            onClick={handleDismissTgBanner}
            className="flex-shrink-0 p-1.5 text-sky-400 hover:text-sky-600 rounded-lg transition-colors"
            title="Скрыть"
          >
          ...
          <Card className="p-6">
            ...
            {user.has_telegram ? (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Уведомления о заказах приходят в Telegram
                  </p>
                </div>
              </div>
            ) : (
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="text-sm text-gray-600">Telegram не привязан</p>
                  <p className="text-xs text-gray-400 mt-0.5">
                    Привяжите Telegram для получения уведомлений о заказах
                  </p>
                </div>
                <a
                  href={telegramBotUrl}
